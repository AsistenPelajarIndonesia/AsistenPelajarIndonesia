...
const getNodeColor = (type: string) => {
  const colors: { [key: string]: string } = {
    root: "#4ade80", // Brighter green for dark theme
    concept: "#60a5fa", // Brighter blue
    subconcept: "#e879f9", // Brighter purple
    definition: "#fb923c", // Brighter orange
    example: "#f87171", // Brighter red
  };
...
const createMindMap = () => {
  ...
  const links = g
    ...
    .attr("fill", "none")
    .attr("stroke", "#525252") // Darker gray for links
  ...
  const tooltip = d3
    ...
    .style("background-color", "#1f2937") // Dark gray background
    .style("padding", "10px")
  ...
  nodes
    ...
    .attr("stroke", "#374151") // Dark gray stroke
    .attr("stroke-width", 2)
    .on("mouseover", (event, d) => {
      ...
      tooltip
        .html(
          `
        <div class="font-bold text-sm mb-1 text-gray-100">${d.data.title}</div>
        <div class="text-xs text-gray-300">${d.data.content || "No description available"}</div>
        <div class="text-xs text-gray-400 mt-1">Type: ${d.data.type}</div>
      `
        )
        ...
        (rel: { target_id: string; type: string }) => {
          ...
          if (target) {
            g.append("path")
              ...
              .attr("stroke", "#525252") // Darker gray for relationships
              .attr("stroke-width", 1)
...
<template>
  <div class="min-h-screen w-full bg-gray-900 p-4">
    <div
      ref="mindmapContainer"
      class="w-full h-[calc(100vh-2rem)] border border-gray-700 rounded-lg bg-gray-800 shadow-lg"
    ></div>
  </div>
</template>
