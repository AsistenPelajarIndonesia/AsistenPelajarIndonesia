  ...
  <div class="container mx-auto p-4 md:p-8">
    <NuxtLayout name="default">
      <div class="max-w-3xl mx-auto">
        <Card class="mb-8">
          <CardHeader>
            <CardTitle class="text-2xl font-bold text-center">SNBT - Penalaran Umum (Penalaran Deduktif)</CardTitle>
            <CardDescription class="text-center">
              Uji kemampuan penalaran deduktif Anda. Waktu pengerjaan: {{ formatTime(timeLeft) }}.
            </CardDescription>
          </CardHeader>
          ...
          <CardContent v-if="showResults && quizResults">
            <h2 class="text-xl font-bold mb-4 text-center">Hasil Simulasi</h2>
            ...
            <Accordion type="single" collapsible class="w-full">
              <AccordionItem v-for="(question, index) in quiz?.questions" :key="question.id" :value="`item-${index}`">
                <AccordionTrigger :class="{'text-green-600': quizResults.answers[question.id]?.isCorrect, 'text-red-600': !quizResults.answers[question.id]?.isCorrect && quizResults.answers[question.id]?.selected}">
                  Soal {{ index + 1 }}: {{ question.questionType }} - {{ quizResults.answers[question.id]?.isCorrect ? 'Benar' : (quizResults.answers[question.id]?.selected ? 'Salah' : 'Tidak Dijawab') }}
                </AccordionTrigger>
                <AccordionContent>
                  <p class="font-medium mb-2">Pertanyaan: {{ question.questionText }}</p>
...
import { useGroqClient, type SnbtPdQuiz, type SnbtPdQuestion, type SnbtPdOption } from '@/composables/useGroqClient';
...
const { generateSnbtPenalaranDeduktifQuestions } = useGroqClient();
const { toast } = useToast();

const quiz = ref<SnbtPdQuiz | null>(null);
...
const timeLeft = ref(10 * 60); // 10 minutes in seconds
let timerInterval: NodeJS.Timeout | null = null;

useHead({
  title: 'SNBT Penalaran Deduktif | Akademi Pelajar',
  meta: [
    { name: 'description', content: 'Latihan soal SNBT Penalaran Umum bagian Penalaran Deduktif dengan AI.' },
  ],
});
...
async function fetchQuizData() {
  ...
  try {
    const data = await generateSnbtPenalaranDeduktifQuestions();
    if (data && data.questions.length > 0) {
      ...
      toast({
        ...
        description: 'Tidak dapat mengambil soal dari server. Silakan coba lagi.',
        variant: 'destructive',
      });
    }
  } catch (error) {
...
function getOptionText(question: SnbtPdQuestion, optionId?: string): string {
